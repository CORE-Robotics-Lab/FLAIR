...
from inverse_rl.models.dmsrd_distill import AIRLMultiStyleDynamicDistill
...
class DMSRDDistill(DMSRD):
    """
    Dynamic Multi Style Reward Distillation for slow distillation to task reward
    """
    ...
    def build_graph(self, iteration):
        ...
        for skill in range(self.num_skills - 1):
            with tf.variable_scope(f"iter_{iteration}_skill_{skill}"):
                ...
                irl_model = AIRLMultiStyleDynamicDistill(self.env, self.task_reward,
        ...
        with tf.Session(config=config) as sess:
            ...
            if len(self.strategy_rewards) >= 4:
                for epoch in range(num_epochs):
                    for skill in range(self.num_skills-1):
                        with rllab_logdir(algo=self.algos[skill], dirname=self.log_path + '/skill_%d' % skill):
                            self.reward_fs[skill].distill(self.policies[0])
                for epoch in range(num_epochs):
...
def main():
    ...
    dmsrd = DMSRDDistill(env, demonstrations, l2_reg_skill=0.001, l2_reg_task=0.001, log_prefix='inverted_pendulum_dmsrd_distill')
