...
from inverse_rl.models.dmsrd_virtual import new_likelihood, AIRLMultiStyleDynamic, Gaussian_Sum_Likelihood, ReLUModel, grid_objective, Grid_Search, GeometricMixturePolicies
...
class DMSRDGAUSSIANMIX(DMSRD):
    ...
    def mixture_finding(self, iteration):
        ...
        for idx, var in enumerate(
            ...
            self.add_dictionary[f'my_skill_{new_strat_ind}_{idx}'] = var
        ...
        for idx, var in enumerate(
            ...
            self.add_dictionary[f'my_value_{new_strat_ind}_{idx}'] = var
        ...
        self.cluster_weights.append([0.0] * len(self.policies))
    ...
    def build_graph(self, iteration):
        ...
        for demo_ind in range(self.num_skills - 1):
            with tf.variable_scope(f"iter_{iteration}_skill_{demo_ind}"):
                irl_var_list = self.task_vars + self.skill_vars[demo_ind] + self.value_vars[demo_ind]
                skill_var_list = self.skill_vars[demo_ind] + self.value_vars[demo_ind]
                ...
                irl_model = AIRLMultiStyleDynamic(self.env, self.task_reward,
                                                  self.strategy_rewards[demo_ind],
                                                  self.value_fs[demo_ind],
                                                  skill_ind=indices_to_train[0] if indices_to_train.shape[0] == 1 else -1,
                                                  ...
                                                  mix_pols=[(GeometricMixturePolicies(self.env.spec, np.array(self.policies)[self.demo_indices_to_train[ind_demo]],
                                                       np.array(self.cluster_weights[ind_demo])[self.demo_indices_to_train[ind_demo]])) if self.demo_indices_to_train[ind_demo].shape[0] > 1 else self.policies[self.demo_indices_to_train[ind_demo][0]] for ind_demo in range(self.num_skills-1)],
                ...
                if opt_pol:
                    set_pol = GeometricMixturePolicies(self.env.spec, np.array(self.policies)[indices_to_train],
    ...
    def first_training_itr(self, iteration):
        ...
        with tf.Session(config=config) as sess:
            ...
            with rllab_logdir(algo=self.algos[0], dirname=self.log_path + '/mixture'):
                ...
                logger.record_tabular(f'Euclidean_Distance', self.best_likelihood)
                logger.record_tabular(f'Euclidean_Mix', self.best_mixture)
                logger.record_tabular(f'KL_Mix', type_mix)
            ...
            self.save_dictionary.update(self.previous_dictionary)
            ...
            if not ratio or self.best_likelihood < 0:
                ...
                self.previous_dictionary = self.add_dictionary
                ...
                self.save_dictionary.update(self.add_dictionary)
                self.new_pol = True
    ...
    def training_itr(self, iteration):
        ...
        with tf.Session(config=config) as sess:
            ...
            with rllab_logdir(algo=self.algos[0], dirname=self.log_path + '/mixture'):
                new_weights = []
                new_dist = []
                for skill in self.demo_readjust:
                    indices_to_train = self.demo_indices_to_train[skill]
                    if indices_to_train.shape[0] > 1:
                        we = np.zeros_like(self.cluster_weights[skill])
                        new_we = self.algos[skill].policy.get_mixture()
                        new_we = np.clip(new_we, 0, None)
                        new_we /= np.sum(new_we)
                        i = 0
                        for idx in indices_to_train:
                            we[idx] = new_we[i]
                            i += 1
                        self.cluster_weights[skill] = (we / np.sum(we)).tolist()
                        new_weights.append(self.cluster_weights[skill])

                        expert = self.experts_multi_styles[skill][0]["observations"]
                        policy = self.algos[skill].policy
                        n, d = expert.shape
                        expert = add_noise(expert)
                        m = 1000
                        const = np.log(m) - np.log(n - 1)
                        diff = np.zeros((1000, expert[0].shape[0]))
                        obs = self.env.reset()
                        for timestep in range(1000):
                            act = policy.get_action(obs)[0]
                            obs, rewards, dones, env_infos = self.env.step(act)
                        ...
                        nnp = query_tree(expert, diff, 3 - 1)
                        nn = query_tree(expert, expert, 3)
                        new_dist.append(const + d * (np.log(nnp).mean() - np.log(nn).mean()))

                logger.record_tabular(f'New_Weights', new_weights)
                logger.record_tabular(f'New_Dist', new_dist)
                logger.dump_tabular(with_prefix=False, write_header=True)
...
def main():
    ...
    dmsrd = DMSRDGAUSSIANMIX(env, demonstrations, grid_shots=2000, log_prefix='inverted_pendulum_dmsrd_virtual')
    #dmsrd.rand_idx = [0, 3, 8, 5, 7, 2, 6, 9, 1, 4]
    dmsrd.rand_idx = [3, 7, 5, 6, 0, 8, 1, 9, 4, 2]

    dmsrd.n_epochs = 800
    dmsrd.add_dictionary = {}
    dmsrd.previous_dictionary = {}
