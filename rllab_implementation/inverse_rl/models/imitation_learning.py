...
from inverse_rl.utils.math_utils import gauss_log_pdf, categorical_log_pdf, numerical_integral, numerical_mixture_integral
...
class SingleTimestepIRL(ImitationLearning):
    ...
    def eval_mixture_probs(self, expert_paths, weights, policies, insert=False, fix=True):
        ...
        for path in expert_paths:
            ...
            if fix:
                var = np.square(np.exp(np.ones_like(var) * -2.7671251))

            new_var = var + actions_square - np.square(actions)

            log_std = np.log(np.clip(np.sqrt(new_var), 1e-37, None))

            path['agent_infos'] = {'mean': actions, 'log_std': log_std}
        return self._compute_path_probs(expert_paths, insert=insert, fix=False)
