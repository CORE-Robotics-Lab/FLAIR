...
class IRLBatchPolopt(RLAlgorithm, metaclass=Hyperparametrized):
    ...
    def __init__(
        ...
        self.optimize_pol = optimize_pol
    ...
    def compute_irl(self, paths, itr=0):
        with logger.tabular_prefix("IRL | "):
            ...
            logger.record_tabular('OriginalTaskSumPosReturn', tot_positive_confidence_rew / float(positive_confidence_num))
            ...
            if self.train_irl:
                ...
                mean_loss = self.irl_model.fit(paths, policy=self.policy, itr=itr, max_itrs=max_itrs, lr=self.reward_lr,
                                               logger=logger, batch_size=self.reward_batch_size)
                ...
                if self.center_grads:
                    if self.center_reward_gradients is None:
                        ...
                        for key in self.center_reward_gradients.keys():
                            self.center_reward_gradients[key] += self.irl_model.center_reward_gradients[key]
                samples_data = self.process_samples(itr, paths)

                logger.log("Logging diagnostics...")
                self.log_diagnostics(paths)
                logger.log("Optimizing policy...")
                if self.optimize_pol:
