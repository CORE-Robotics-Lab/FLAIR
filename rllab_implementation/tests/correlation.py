...
if __name__ == "__main__":
    ...
    ax.scatter(dataframe["Ground Truth Reward"], dataframe["AIRL Batch"],s=1.5,label=r'AIRL Batch: $r=0.776$')
    ...
    ax.scatter(dataframe["Ground Truth Reward"], dataframe["Center"],s=1.5,label=r"MSRD: $r=0.917$")
    ...
    ax.scatter(dataframe["Ground Truth Reward"], dataframe["DMSRD Rescaled Task Reward"],s=1.5,label=r"DMSRD (Ours): $r=0.966$")

    # ax.plot(dataframe["Ground Truth Reward"], intercept + slope * dataframe["Ground Truth Reward"], 'r', color="black")
    # ax.legend([f'R = {round(r_value,3)}'])

    ax.plot(range(-1570, 0, 1), range(-1570, 0, 1), color="black", linestyle="--")
    ...
    plt.savefig("data/figs/task_correlation_test_final.png")
    plt.close(fig)
