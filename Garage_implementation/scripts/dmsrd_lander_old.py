...
class DMSRD:
    ...
    """
    def __init__(self, env, env_test, demonstrations, log_prefix, crp_alpha=50, state_only=True, n_epochs=1000, msrd_epochs=1000, mixture_starts=2,
        ...
        with tf.Session(config=config) as sess:
            for skill in range(self.num_skills):
                ...
                if skill == 0:
                    ...
                    for idx, var in enumerate(
                            tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=f'policy_{skill}')):
                        self.new_dictionary[f'my_policy_{skill}_{idx}'] = var
        ...
        with tf.Session(config=config) as sess:
            ...
            if iteration == len(self.demonstrations)-1:
                ...
                likelhi = get_dmsrd_likelihood(self.experts_multi_styles,
                                               ...
                                               self.reward_fs[0],
                                               self.cluster_weights)
                rew, div = get_dmsrd_divergence(env_test, self.cluster_weights,
                                                self.policies,
                                                self.experts_multi_styles, 500)
                ...
                trajectories = []
                for i in range(40):
                    with open(f'data/lunar_trajs/trajectories_{i}.pkl',
                              "rb") as f:
                ...
                for tidx, traj in enumerate(trajectories):
                    reward_cent = tf.get_default_session().run(
                        ...
                        feed_dict={
                            self.reward_fs[0].obs_t: traj["observations"]})
                    score = reward_cent[:, 0]
                    ...
                    for strat in range(len(self.reward_fs)):
                        ...
                        for traj in demo:
                            reward = tf.get_default_session().run(
                                ...
                                feed_dict={
                                    self.reward_fs[strat].obs_t: traj[
                                        "observations"]})
                ...
                for j in range(len(rew[0])):
                    rew[:, j] = (rew[:, j] - np.min(rew[:, j])) / np.ptp(
                        rew[:, j])

                rew_nomix = rew[list(self.skill_to_demo.keys())]
                name = [f'Demonstration {i}' for i in [0, 1, 3, 4, 6, 7, 9]]
...
def main():
    ...
    dmsrd = DMSRD(env, env_test, demonstrations, grid_shots=1500, gen_update_step=2, render_video=True, log_prefix='lunar_lander_dmsrd')
    # dmsrd.rand_idx = [0, 3, 8, 5, 7, 2, 6, 9, 1, 4]
    # dmsrd.rand_idx = [3, 7, 5, 6, 0, 8, 1, 9, 4, 2]

    dmsrd.n_epochs = 700
    dmsrd.msrd_epochs = 1000
