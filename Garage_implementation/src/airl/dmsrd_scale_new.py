...
class DMSRDScale:
    ...
    def msrd(self, iteration, no_train=False):
        ...
        with self._create_tf_session() as sess:
            ...
            if len(self.strategy_rewards) >= self.start_msrd and self.new_pol and not no_train:
                # MSRD
                for strategy in range(self.num_strategies):
            ...
            for i in range(40):
                with open(
                    f'data/{self.trajs_path}/trajectories_{i}.pkl',
                    "rb") as f:
            ...
            likelhi = get_dmsrd_likelihood(
                self.experts_multi_styles,
                self.policies,
                self.reward_fs[0],
                self.mixture_weights)
            likes.append(np.mean(likelhi))
            ...
            grounds.append(
                np.corrcoef(ground_truths, record)[0, 1])
