...
class GaussianMLPPolicy(GaussianMLPModel, Policy):
    ...
    def _initialize_policy(self):
        ...
        state_input = tf.compat.v1.placeholder(tf.float32,
                                               shape=(None, None,
                                                      self._obs_dim))
        self.dist, self.mean, self.log_std = self.build(state_input).outputs
    ...
    def get_actions(self, observations):
        ...
        samples, means, log_stds = tf.compat.v1.get_default_session().make_callable(
            [
                self.dist.sample(seed=deterministic.get_tf_seed_stream()),
                self.mean, self.log_std
            ],
            feed_list=[(np.expand_dims(
            observations, 1))])
